...
// G£à local UI photo type
import { UPhoto } from "@/lib/types";

// G£à photos client (DB + Cloudinary)
import {
  listPhotos,
  createPhoto,
  updatePhoto,
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        console.log("G£à Photos loaded for report:", form.reportId);
      } catch (e) {
  ...
  function makePersistingSetter(sectionKey: keyof typeof sectionPhotos) {
    return async (nextArr: DBUPhoto[]) => {
      ...
      for (const p of nextArr) {
        if (!p._id) {
          // New GåÆ upload to Cloudinary via API and save
          ...
          // Existing GåÆ only update if metadata changed
              ...
              <div className="space-y-4">
                {(
                  [
                    ...
                    ["inspectorSummary", "Inspector's Summary (short)", "1GÇô3 concise bullet pointsGÇª"],
