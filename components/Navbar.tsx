...
export default function Navbar() {
  ...
  useEffect(() => {
    if (status === "unauthenticated") { setProfile(null); setOpen(false); setMenuOpen(false); }
  }, [status]);
  ...
  const NavLink = ({ href, children }: { href: string; children: React.ReactNode }) => {
    ...
    return (
      <Link
        ...
        ${active ? "bg-[#78c850]/10 text-[#78c850] shadow-sm" : "text-gray-700 hover:bg-[#78c850]/5 hover:text-[#78c850]"}`}
      >
        {children}
        {active && <span className="absolute -bottom-0.5 left-1/2 -translate-x-1/2 w-6 h-0.5 bg-[#78c850] rounded-full" />}
      </Link>
    );
  };
  ...
  return (
    <>
      {/* Fixed header with safe-area + no horizontal scroll on small screens */}
      <header className="fixed top-0 left-0 right-0 z-50 w-full overflow-x-clip border-b border-[#78c850]/20 bg-white/90 backdrop-blur supports-[backdrop-filter]:bg-white/70">
        <nav className="w-full mx-auto max-w-screen-xl px-3 sm:px-5 lg:px-8 h-14 md:h-16 flex items-center justify-between gap-2">
          {/* Left: Logo */}
          <Link href="/" className="flex items-center gap-2 min-w-0 shrink-0" aria-label="Ninekiwi Home">
            ...
            <span className="hidden xs:block text-lg sm:text-xl md:text-2xl font-extrabold tracking-tight truncate max-w-[44vw]">
            ...
            {!user ? (
              ...
              <div className="relative">
                ...
                >
                  ...
                  <span className="text-sm font-medium text-gray-700">{`Hi, ${(profile?.name || user.name)?.split(" ")[0]}`}</span>
                  <svg className={`w-4 h-4 text-[#78c850] transition-transform ${menuOpen ? "rotate-180" : ""}`} viewBox="0 0 20 20" fill="currentColor">
                ...
                {menuOpen && (
                  <>
                    ...
                    <div className="absolute right-0 mt-2 w-72 bg-white border border-[#78c850]/20 rounded-2xl shadow-2xl z-50 overflow-hidden animate-in fade-in slide-in-from-top-2">
                      {user && (
                      ...
                      <div className="py-1">
                        <Link href="/account" className="flex items-center gap-3 px-4 py-2.5 text-sm text-gray-700 hover:bg-[#78c850]/5">
                          <svg className="w-4 h-4 text-[#78c850]" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
                          </svg>
                          Account Settings
                        </Link>
                        <Link href="/early-access" className="flex items-center gap-3 px-4 py-2.5 text-sm text-gray-700 hover:bg-[#78c850]/5">
                          <svg className="w-4 h-4 text-[#78c850]" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        ...
                        {user?.role === "admin" && (
                          <Link href="/admin" className="flex items-center gap-3 px-4 py-2.5 text-sm text-gray-700 hover:bg-[#78c850]/5">
                            <svg className="w-4 h-4 text-[#78c850]" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          <button
            className="md:hidden ml-auto text-[#78c850] p-2 rounded-xl hover:bg-[#78c850]/10 transition-colors shrink-0"
            onClick={() => setOpen((o) => !o)}
        ...
        {/* Mobile sheet menu (fixed, full width) */}
        <div
          id="mobile-menu"
          aria-hidden={!open}
          className={`md:hidden fixed top-14 left-0 right-0 z-40 border-t border-[#78c850]/20 bg-white/95 backdrop-blur transition-transform duration-300 ease-out
        ...
        >
          <div className="mx-auto max-w-screen-xl px-3 sm:px-5 py-4 space-y-3">
            ...
            <div className="grid gap-2">
              <Link href="/" className="block px-4 py-3 rounded-lg border border-[#78c850]/30 text-sm font-medium text-gray-700 hover:bg-[#78c850]/5">Home</Link>
              <Link href="/contact" className="block px-4 py-3 rounded-lg border border-[#78c850]/30 text-sm font-medium text-gray-700 hover:bg-[#78c850]/5">Contact</Link>
              {user && (
                <Link href="/account" className="block px-4 py-3 rounded-lg border border-[#78c850]/30 text-sm font-medium text-gray-700 hover:bg-[#78c850]/5">Account</Link>
              )}
              {user?.role === "admin" && (
                <Link href="/admin" className="block px-4 py-3 rounded-lg border border-[#78c850]/30 text-sm font-medium text-gray-700 hover:bg-[#78c850]/5">Admin</Link>
              )}
            </div>
            ...
            {!user ? (
              <div className="grid gap-2 pt-1">
                ...
                <Link href="/signup" className="inline-flex items-center justify-center rounded-lg bg-[#78c850] hover:bg-[#66b343] text-white font-medium transition-all px-4 py-2.5 text-sm">
                  Sign up
                </Link>
