...
export default function SectionPhotos({ title, photos, setPhotos, summaryToggle, section }: Props) {
  ...
  const renumber = useCallback((arr: UPhoto[]) => arr.map((p, i) => ({ ...p, figureNumber: i + 1, section })), [section]);
    ...
    <div className="bg-white rounded-xl border-2 border-gray-200 p-6 shadow-sm">
      ...
      <div className="space-y-4">
        {safePhotos.map((p, idx) => (
          <div key={idx} className="border-2 border-gray-200 rounded-lg p-4 bg-gray-50 hover:border-kiwi-green transition-colors">
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img src={p.data} alt={p.name} className="w-full h-48 object-cover rounded-lg mb-3 shadow-sm" />
            ...
            <div className="bg-white p-4 rounded-lg space-y-3">
              ...
              <div>
                ...
                <input
                  ...
                  value={p.caption ?? ""}
                  className="w-full border-2 border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-kiwi-green focus:border-transparent"
                  onChange={(e) => {
                    const next = [...safePhotos];
                    next[idx] = { ...next[idx], caption: e.currentTarget.value ?? "" };
