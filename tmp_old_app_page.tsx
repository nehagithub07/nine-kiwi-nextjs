...
// G£‡ local UI photo type
import { UPhoto } from "@/lib/types";

// G£‡ photos client (DB + Cloudinary)
import {
  listPhotos,
  createPhoto,
  updatePhoto,
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        console.log("G£‡ Photos loaded for report:", form.reportId);
      } catch (e) {
  ...
  function makePersistingSetter(sectionKey: keyof typeof sectionPhotos) {
    return async (nextArr: DBUPhoto[]) => {
      ...
      for (const p of nextArr) {
        if (!p._id) {
          // New GÂ∆ upload to Cloudinary via API and save
          ...
          // Existing GÂ∆ only update if metadata changed
              ...
              <div className="space-y-4">
                {(
                  [
                    ...
                    ["inspectorSummary", "Inspector's Summary (short)", "1G«Ù3 concise bullet pointsG«™"],
